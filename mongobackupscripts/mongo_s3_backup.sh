...
mongodump -h localhost:27017 --username=root --password=mypassword --authenticationDatabase=admin -d go-mongodb -o $MONGO_BACKUP_FILENAME

aws s3 cp $MONGO_BACKUP_FILENAME s3://my-tf-mongo-backup-bucket-for-wiz-testing-environment-akmal/$(date +%m-%d-%y) --recursive
